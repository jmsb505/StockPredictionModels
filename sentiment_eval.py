...
for arch in archs:
    ...
    mean=np.mean(sent)
    std=np.std(sent)
    noise=np.random.uniform(0, 1,len(sent))
    df['p_sentiment']=noise
    ...
    metrics_df.to_csv(project_path+f'/Results/fmetric/{arch}_metrics.csv',index=False)
    std_metrics_df.to_csv(project_path+f'/Results/fmetric/{arch}_std_metrics.csv',index=False)
    metrics=[]
    df_dict={}
    ...
    # %%
        ...
        if arch=='bilstm':
            ...
            archUp=arch.upper()
        tuner = kt.GridSearch(builds[archs.index(arch)],
                            objective='val_loss',
        ...
        dir=project_path+f'/Graphs_f/{arch}/'+ticker
        ...
        for train_index, test_index in tscv.split(X):
            ...
            if counter_fold==11:
                ...
                model.build((None, 7, 17))
                model.load_weights(f'/home/j/usfq/Proyecto-Integrador/StockPredictionModels/Models/{archUp}/Weights/model_{arch}_{ticker}.h5')
                history=model
    ...
    df_ps['RMSE']=rmse_t_p
    df_ps['MPE']=mpe_t_p
    df_ps.to_csv(project_path+f'/Results/fmetric/{arch}_hypothesis.csv',index=False)
    ...
    df_loss.to_csv(project_path+f'/Results/fmetric/{arch}_loss.csv',index=False)
